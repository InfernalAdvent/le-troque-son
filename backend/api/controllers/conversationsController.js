...
const conversationController = {
  ...
  getConversationHistory: async (req, res) => {
    ...
    try {
      ...
      if (markedAsRead.length > 0) {
        const io = getIo();
        const senderIds = [
          ...new Set(markedAsRead.map((m) => m.expediteur_id)),
        ];
  ...
  postMessage: async (req, res) => {
    const { error: paramError, value: paramValue } =
      conversationIdSchema.validate({ id: parseInt(req.params.id, 10) });
    if (paramError) {
      return res.status(400).json({ error: paramError.details[0].message });
    }

    const { error: bodyError, value: bodyValue } = postMessageSchema.validate(
      req.body,
    );
    if (bodyError) {
    ...
    try {
      const { newMessage, recipientId } = await conversationService.sendMessage(
        conversationId,
        userId,
        contenu,
      );
