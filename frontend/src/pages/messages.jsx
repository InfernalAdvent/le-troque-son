      ...
      try {
        ...
        if (createConvData) {
          ...
          const existingConv = res.data.find(
            (c) => c.annonce_id === createConvData.annonceId,
          );
          ...
          if (existingConv) {
            ...
            try {
              const annonceRes = await api.get(
                `/annonces/${createConvData.annonceId}`,
              );
              const vendeurRes = await api.get(
                `/users/${createConvData.vendeurId}`,
              );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="bg-white rounded-xl shadow-md overflow-hidden flex h-[70vh]">
        ...
        <div
          className={`w-full md:w-1/3 border-r border-gray-200 overflow-y-auto ${selectedConversation ? "hidden md:block" : "block"}`}
        >
          {conversations.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-500 p-6 text-center">
              <p className="text-lg font-medium">Aucune conversation</p>
              <p className="text-sm mt-2">
                Contactez un vendeur depuis une annonce pour démarrer une
                conversation
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p
                        className={`text-sm text-gray-700 truncate mt-0.5 ${unread ? "font-bold" : ""}`}
                      >
                        {conversation.messages?.[0] ? (
                          <>
                            <span className="text-gray-600">
                              {conversation.messages[0].expediteur_id ===
                              user?.id
                                ? "Vous : "
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <p
                              className={`text-xs mt-1 text-right ${isOwn ? "text-green-200" : "text-gray-500"}`}
                            >
