  ...
  const handleSaveWishlist = async () => {
    try {
      ...
      if (wishlistRes.data.length > 0) {
        setWishlistText(
          wishlistRes.data.map((item) => item.souhait_texte).join("\n"),
        );
      } else {
        setWishlistText("");
        setEditingWishlist(false);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
              ...
              {isOwnProfile ? (
                <>
                  <p className="text-gray-700 flex items-center gap-1">
                    <MapPin size={18} />
                    {profileUser.departement?.nom ||
                      profileUser.departement_numero}
                  </p>
                </>
              ) : (
                <>
        ...
        <div className="mt-12">
          <h1 className="text-4xl text-green-600 font-bold mb-6">
            {isOwnProfile
              ? "Mes annonces"
              : `Annonces de ${profileUser.pseudo}`}
          </h1>
