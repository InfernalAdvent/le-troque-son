...
const photoController = {
  ...
  getByAnnonceId: async (req, res) => {
    const { error, value } = annonceIdSchema.validate({
      annonceId: parseInt(req.params.annonceId, 10),
    });
    if (error) {
      return res.status(400).json({ error: error.details[0].message });
    }
  ...
  updateOrder: async (req, res) => {
    const { error: paramError, value: paramValue } = annonceIdSchema.validate({
      annonceId: parseInt(req.params.annonceId, 10),
    });
    if (paramError) {
    ...
    const { error: bodyError, value: bodyValue } = updateOrderSchema.validate(
      req.body,
    );
    if (bodyError) {
    ...
    try {
      ...
      res
        .status(status)
        .json({ error: isForbidden ? error.message : "Erreur serveur." });
    }
  },
};
