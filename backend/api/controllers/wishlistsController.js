...
const wishlistController = {
  createWishlist: async (req, res) => {
    ...
    try {
      ...
      res
        .status(400)
        .json({ error: "Erreur lors de la création de la wishlist." });
    }
  },
  updateWishlistOwner: async (req, res) => {
    ...
    try {
      ...
      const updatedElement = await wishlistService.updateWishlistOwner(
        wishlistId,
        value,
        userId,
      );
      ...
      if (!updatedElement) {
        ...
        return res
          .status(404)
          .json({
            message: "Wishlist non trouvée ou vous n'êtes pas l'auteur.",
          });
      }
