...
const conversationService = {
  ...
  findOrCreateConversation: async (userId, receveurId, annonceId) => {
    ...
    try {
      ...
      const existingConversation = await Conversation.findOne({
        where: {
          ...
          [Op.or]: [
            ...
            {
              utilisateur_initiateur_id: initiatorId,
              utilisateur_receveur_id: recipientId,
            },
            // Cas 2: Receveur est initiateur, User est receveur (pour couvrir les deux sens)
            {
              utilisateur_initiateur_id: recipientId,
              utilisateur_receveur_id: initiatorId,
            },
          ],
        },
      ...
      throw new Error(
        `Erreur lors de la création/recherche de la conversation : ${error.message}`,
      );
    }
  },
      ...
      return await Conversation.findAll({
        where: {
          [Op.or]: [
            ...
            { utilisateur_receveur_id: userId, masquee_par_receveur: false },
          ],
        },
  ...
  getConversationHistory: async (conversationId, userId) => {
    try {
      ...
      if (!isParticipant) {
        throw new Error(
          "Accès refusé. L'utilisateur n'est pas participant à cette conversation.",
        );
      }
  ...
  sendMessage: async (conversationId, userId, contenu) => {
    try {
      ...
      if (!isParticipant) {
        throw new Error(
          "Accès refusé. Vous n'êtes pas autorisé à envoyer un message dans cette conversation.",
        );
      }
  ...
  hideConversation: async (conversationId, userId) => {
    try {
      ...
      const isInitiateur =
        conversation.utilisateur_initiateur_id === parseInt(userId, 10);
      const isReceveur =
        conversation.utilisateur_receveur_id === parseInt(userId, 10);
