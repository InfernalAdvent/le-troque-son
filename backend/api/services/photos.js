...
const photoService = {
  uploadPhotos: async (files, userId, annonceId = null) => {
    try {
      ...
      throw new Error(
        "Erreur lors du téléchargement des photos : " + error.message,
      );
    }
  },
  ...
  updateOrder: async (photoIds, annonceId, userId) => {
    try {
      ...
      await Promise.all(
        photoIds.map(({ id, ordre }) => {
          ...
          if (!photoIdsInDb.includes(id)) {
            throw new Error(
              `Photo ${id} n'appartient pas à l'annonce ${annonceId}`,
            );
          }
      ...
      throw new Error(
        "Erreur lors de la mise à jour de l'ordre : " + error.message,
      );
    }
  },
};
