...
function Home() {
  ...
  return (
    <>
      <Helmet>
        <title>
          Le Troque Son | Achetez et échangez vos instruments de musique, vos
          vinyles et vos CDs !
        </title>
        <meta
          name="description"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-black text-left mb-4">
          {searchQuery
            ...
            : "En ce moment sur Le Troque Son"}
        </h1>

        <p className="text-gray-700 mb-4">
          {annonces.length} annonce{annonces.length > 1 ? "s" : ""} trouvée
          {annonces.length > 1 ? "s" : ""}
        </p>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const queryParams = [];
        if (departementsParam)
          queryParams.push(`departements=${departementsParam}`);
      ...
      <main className="flex-1">
        ...
        >
          <Routes>
            ...
            <Route
              path="/mon-profil"
              element={<UserProfile key="own-profile" />}
            />
