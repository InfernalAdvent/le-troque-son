...
const updateAnnonceSchema = Joi.object({
  ...
  etat: Joi.string()
    .valid("Comme neuf", "Très bon état", "Bon état", "Usagé")
    .optional(),
...
const filtersSchema = Joi.object({
  departements: Joi.string()
    ...
    .messages({
      "string.pattern.base":
        "Les départements doivent être des numéros séparés par des virgules.",
    }),
...
const categoryFiltersSchema = Joi.object({
  filtres: Joi.string()
    ...
    .messages({
      "string.pattern.base":
        "Les filtres doivent être des IDs séparés par des virgules.",
    }),
  departements: Joi.string()
    .pattern(/^\d{2,3}(,\d{2,3})*$/)
    .optional()
...
const annonceController = {
  ...
  getAnnoncesByCategories: async (req, res) => {
    // Valider les params
    const { error: paramError } = categoriesSchema.validate({
      id: req.params.id,
    });
    if (paramError) {
    ...
    const { error: queryError, value: queryValue } =
      categoryFiltersSchema.validate(req.query, { stripUnknown: true });
    if (queryError) {
  ...
  getAllWithFilters: async (req, res) => {
    ...
    try {
      ...
      const annonces = await annoncesService.getAllWithFilters({
        departements,
        search,
      });
      res.json(annonces);
  ...
  updateAnnonceOwner: async (req, res) => {
    ...
    try {
      ...
      const updatedElement = await annoncesService.updateAnnonceOwner(
        annonceId,
        value,
        userId,
      );
      ...
      if (!updatedElement) {
        return res
          ...
          .json({
            message:
              "Vous devez modifier votre annonce avant de l'enregistrer.",
          });
      }
  ...
  deleteAnnonce: async (req, res) => {
    // Valider les params
    const { error } = deleteAnnonceSchema.validate({
    ...
    try {
      ...
      if (!userId) {
        return res
          .status(401)
          .json({
            error: "Authentification requise pour supprimer cette annonce.",
          });
      }
      ...
      if (!deletedAnnonce) {
        return res
          ...
          .json({
            message:
              "Annonce non trouvée ou vous ne disposez les droits pour supprimer cette annonce",
          });
      }
      res.status(204).send();
    } catch (error) {
